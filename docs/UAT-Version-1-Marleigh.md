...
10. Confirm `/api/version` reports app version `1.4.3` after this patch is installed.
11. Confirm the app footer also shows version `1.4.3`.
12. Confirm the Review source section shows EMR/API access and Manual upload.
13. Open Treatment plans.
14. Confirm the updated evidence queue banner is visible and the footer shows `v1.4.3`.
...
33. Review API settings, stored API endpoint profiles, Alleva REST/OpenAPI URL guidance, and optional LLM settings.
...
43. Confirm the Version 1.4.3 local recovery utility exists at `scripts\update-local-admin.ps1`.
