...
Date: 2026-06-17
Current app patch: `1.4.1` / build `2026.06.18.1`
...
- visible updated evidence queue and footer version `1.4.1` so operators can confirm the refreshed UI is being served
...
The desktop runtime serves `frontend\dist` when present. Version `1.4.1` keeps the Windows preflight stale-build guard so source-checkout launches rebuild the frontend when npm is available and the React source is newer than `frontend\dist`; otherwise preflight warns that the served browser UI may be stale.
...
Version 1.4.1 uses the latest valid treatment-plan review/update date, or admission date when no later valid update exists, as the recurring date-clock anchor. PHP uses 30 calendar days and other configured treatment levels use 60 calendar days. LOC changes use a separate manager-editable 7-calendar-day preset, but that setting remains visibly unvalidated and affected records stay `Needs Review` until R3/Marleigh confirms the final rule.
