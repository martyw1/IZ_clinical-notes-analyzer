...
The component HTTP/API fixture evidence verifies the synthetic beta.3 executable contract and safe stop/listener release. It does not qualify the final package. Final package installation, default-profile, standard-user Windows Home, cross-user recovery (`R07`), and power-loss recovery (`R09`) results remain **Pending/blocked** because the required package, Windows profiles, and infrastructure are not available in this station. No real client records were accessed or authorized.
