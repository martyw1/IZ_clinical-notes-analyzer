...
1. Sign in as `admin`.
2. Confirm the footer shows version `1.0.3` after the final version bump.
3. Open `Treatment plans`.
4. Confirm the dashboard loads, the `Updated evidence queue v1.0.3` banner is visible, and the LOC-change window is visibly unvalidated if not confirmed.
5. Open `Settings`.
...
8. Create a synthetic draft workflow profile with no PHI.
9. Publish the synthetic draft version.
10. Archive the synthetic profile.
...
- Screenshot of the app footer showing version `1.0.3`.
- Screenshot of the Treatment Plan Timeliness tab showing `Updated evidence queue v1.0.3`.
- Screenshot of Settings workflow profiles showing the seeded Treatment Plan Timeliness Tracker profile.
