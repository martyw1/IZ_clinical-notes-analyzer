...
1. Sign in as `admin`.
2. Confirm the footer shows version `1.3.0` after the final version bump.
3. Open `Treatment plans`.
4. Confirm the dashboard loads, the `Updated evidence queue v1.3.0` banner is visible, and the LOC-change window is visibly unvalidated if not confirmed.
5. Open `App settings`.
...
9. Confirm the draft snapshot and transition rules can be edited before publish.
...
- Screenshot of the app footer showing version `1.3.0`.
- Screenshot of the Treatment Plan Timeliness tab showing `Updated evidence queue v1.3.0`.
