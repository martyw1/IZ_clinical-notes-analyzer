...
## Repo purpose
Local-first Windows 10/11 clinical-notes and Treatment Plan Timeliness Tracker app for R3 Recovery Services. The current source candidate is `1.0.0` / build `2026.09.21.2` / installer revision `1` on the `stable-local-desktop` channel. Its exact ZIP passed 799 tests, seven build gates, two fresh component-profile password lifecycles, packaged upgrade/reinstall and normal/OneDrive source checks (see `docs/validation/installer-portability-2026-09-21.md`). Actual client/Windows 10 and full platform qualification remain open; the verified September 15 package remains immutable historical evidence. Normal Windows desktop use must not require Windows administrator access, Docker, PostgreSQL, Git, Node.js, or command-line work when a prepared release folder with built frontend assets is used.
