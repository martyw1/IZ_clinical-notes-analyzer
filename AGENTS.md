...
## Repo purpose
Local-first Windows 10/11 clinical-notes and Treatment Plan Timeliness Tracker app for R3 Recovery Services. The current source candidate is `1.0.0` / build `2026.09.21.1` / installer revision `1` on the `stable-local-desktop` channel. Its relocated-package and OneDrive-backed package flow is under validation (see `docs/validation/installer-portability-2026-09-21.md`); the verified September 15 package remains immutable historical evidence. Normal Windows desktop use must not require Windows administrator access, Docker, PostgreSQL, Git, Node.js, or command-line work when a prepared release folder with built frontend assets is used.
