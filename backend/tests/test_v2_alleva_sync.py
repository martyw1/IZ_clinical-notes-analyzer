...
class _MockAllevaHandler(BaseHTTPRequestHandler):
    paths: list[str] = []
    block_clients = False
    clients_started = Event()
    release_clients = Event()
    ...
    def do_GET(self) -> None:
        ...
        if path == "/clients":
            type(self).clients_started.set()
            if type(self).block_clients:
                type(self).release_clients.wait(timeout=3)
...
@contextmanager
def _mock_alleva_server(*, block_clients: bool = False) -> Iterator[str]:
    server = ThreadingHTTPServer(("127.0.0.1", 0), _MockAllevaHandler)
    thread = Thread(target=server.serve_forever, daemon=True)
    _MockAllevaHandler.paths.clear()
    _MockAllevaHandler.block_clients = block_clients
    _MockAllevaHandler.clients_started.clear()
    _MockAllevaHandler.release_clients.clear()
    thread.start()
    try:
        yield f"http://127.0.0.1:{server.server_port}"
    finally:
        _MockAllevaHandler.release_clients.set()
        _MockAllevaHandler.block_clients = False
        server.shutdown()
        thread.join()
        server.server_close()
...
def test_approved_alleva_sync_reads_mocked_http_and_persists_normalized_aggregate(tmp_path, monkeypatch) -> None:
    ...
    with _mock_alleva_server() as base_url:
        ...
        assert _MockAllevaHandler.paths == ["/token", "/clients?limit=100&offset=0", "/treatment-plans?limit=100&offset=0", "/treatment-plans/plan-912"]
...
def test_approved_alleva_sync_job_can_be_cancelled_while_an_api_page_is_in_flight(tmp_path, monkeypatch) -> None:
    ...
    with _mock_alleva_server(block_clients=True) as base_url:
        configured = client.patch(
            "/api/api-configuration",
            headers=headers,
            json={
        ...
        assert started.status_code == 202
        job_id = started.json()["job_id"]
        assert _MockAllevaHandler.clients_started.wait(timeout=2)
        ...
        assert _MockAllevaHandler.paths == ["/token", "/clients?limit=100&offset=0"]
