...
def _parsed_fields(*, due_date: str, signature_datetime: str = "2026-06-02") -> ParsedManualFields:
...
def test_timing_is_late_when_required_evidence_exists_and_due_date_has_passed() -> None:
    # Given
...
def test_timing_is_within_window_when_required_evidence_exists_and_due_date_has_not_passed() -> None:
    ...
    # Then
    assert evaluation.status == "Within Window"
...
def test_manual_aggregate_import_persists_derived_late_status(tmp_path, monkeypatch) -> None:
    # Given
    client = _fresh_client(tmp_path, monkeypatch)
    headers = _auth_headers(client)
    aggregate = build_manual_aggregate(_parsed_fields(due_date=(date.today() - timedelta(days=1)).isoformat()))
    ...
    assert detail.json()["overall_status"] == "Late"
