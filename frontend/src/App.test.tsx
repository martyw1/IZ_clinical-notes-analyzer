...
describe('V2 active app shell', () => {
  it('requires first-run password change before rendering the workspace', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Confirm new password'), { target: { value: 'SyntheticActivePass456' } })
    fireEvent.click(screen.getByRole('button', { name: 'Update password' }))

    await screen.findByRole('heading', { name: 'Protect your account' })
    fireEvent.change(screen.getByLabelText('Current password for recovery setup'), { target: { value: 'SyntheticActivePass456' } })
