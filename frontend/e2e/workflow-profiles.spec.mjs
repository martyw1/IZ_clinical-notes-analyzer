...
test('admin enables treatment-plan sync and sees a mocked synced treatment plan', async ({ page }, testInfo) => {
  ...
  await page.route((url) => url.pathname.startsWith('/api/'), async (route) => {
    ...
    if (path === '/api/api-configuration') return respond({ vendor_name: 'Mock Alleva', api_base_url: 'http://mock.invalid', openapi_url: 'http://mock.invalid/openapi.json', token_url: 'http://mock.invalid/token', client_id: 'mock-client', api_key_configured: syncEnabled, client_secret_configured: true, token_auth_style: 'body', scopes: '', pagination_limit: 100, sync_limit: 100, requests_per_minute: 600, timeout_seconds: 10, api_enabled: syncEnabled, treatment_plan_sync_enabled: syncEnabled, treatment_plan_sync_approved: syncEnabled, active_contract_version: null })
  ...
  await page.getByRole('button', { name: 'Treatment Plans' }).click()
  await page.getByRole('button', { name: 'plan-912' }).click()
...
test('admin can cancel a queued treatment-plan sync', async ({ page }, testInfo) => {
  ...
  await page.route((url) => url.pathname.startsWith('/api/'), async (route) => {
    ...
    if (path === '/api/v2/dashboard') return respond({ source_cards: [], metrics: {}, blockers: [] })
    if (path === '/api/settings') return respond({ organization_name: 'R3 Recovery Services', facility_timezone: 'America/New_York', treatment_plan_master_due_days: 30, treatment_plan_php_review_interval_days: 30, treatment_plan_iop_op_review_interval_days: 90, treatment_plan_loc_change_window_days: 7, treatment_plan_loc_change_window_validated: false })
    if (path === '/api/api-configuration') return respond({ vendor_name: 'Mock Alleva', api_base_url: 'http://mock.invalid', openapi_url: 'http://mock.invalid/openapi.json', token_url: 'http://mock.invalid/token', client_id: 'mock-client', api_key_configured: true, client_secret_configured: true, token_auth_style: 'body', scopes: '', pagination_limit: 100, sync_limit: 100, requests_per_minute: 600, timeout_seconds: 10, api_enabled: true, treatment_plan_sync_enabled: true, treatment_plan_sync_approved: true, active_contract_version: null })
