...
const queueItems = [
  ...
  },
  {
    ...
    status: 'Current/Compliant', missing_criteria_count: 0, returned_criteria_count: 0,
    source_mode: 'alleva_rest_api', warnings: [],
  },
]
...
function planDetail(planId) {
  return {
    ...
    overall_status: planId === 'plan-913' ? 'Current/Compliant' : 'Needs Review',
    ...
    evidence_coverage_summary: { criteria_total: 42, criteria_with_evidence: 1, criteria_missing_evidence: 0, runtime_only_fields: [] },
    content_snapshot: {
      plan_id: planId,
...
test('full treatment-plan pull populates both operational tabs and keeps multiple plans selectable', async ({ page }, testInfo) => {
  ...
  await page.route((url) => url.pathname.startsWith('/api/'), async (route) => {
    ...
    if (path === '/api/v2/treatment-plans') return respond({ items: synced ? queueItems : [], status_order: ['Needs Review', 'Current/Compliant'] })
    ...
    if (path === '/api/v2/patient-roster') return respond({ items: synced ? [{ patient_id: '912', source_mode: 'alleva_rest_api', lifecycle_state: 'active', current_level_of_care: 'PHP', treatment_plan_id: 'plan-913', treatment_plan_status: 'Current/Compliant', first_seen_at: '2026-07-13T10:00:00Z', last_seen_at: '2026-07-13T10:05:00Z', reconciled_at: '2026-07-13T10:05:00Z' }] : [] })
  ...
  await page.screenshot({ path: testInfo.outputPath('treatment-plans-multiple-plans.png'), fullPage: true })
  ...
  await expect(page.getByText('Current/Compliant')).toBeVisible()
  await expect(page.getByText(/Patient names are excluded/i)).toBeVisible()
  await page.screenshot({ path: testInfo.outputPath('patient-roster-after-pull.png'), fullPage: true })
})
